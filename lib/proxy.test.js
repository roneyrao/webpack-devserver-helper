...
describe('parseProxies', () => {
  ...
  it("{'api':'http://afsfa.com'}", () => {
    ...
    expect(result).to.eql({
      [newName]: Object.assign({ target: proxies[name], pathRewrite: { [`^${newName}`]: '' } }, log),
    });
  });
  ...
  it("{context: ['/auth/', '/api'],target: 'http://localhost:3000'}", () => {
    ...
    const newContext = proxies.context.map(name => `${publicPath}${name.endsWith('/') ? name.substr(0, name.length - 1) : name}`);
    const pathRewrite = {};
    newContext.forEach((name) => { pathRewrite[`^${name}`] = ''; });
    const result = parseProxies(proxies, publicPath, apiPrefix);
    expect(result).to.eql([
  ...
  it("{context: ['/auth/', '/api'],target: 'http://localhost:3000', pathRewrite:{abc:1}}} - pathRewrite is untouched", () => {
    const proxies = { context: ['/auth/', '/api'], target: 'http://localhost:3000', pathRewrite: { abc: 1 } };
    const newContext = proxies.context.map(name => `${publicPath}${name.endsWith('/') ? name.substr(0, name.length - 1) : name}`);
    const result = parseProxies(proxies, publicPath, apiPrefix);
    expect(result).to.eql([
      Object.assign(proxies, { context: newContext }, log),
    ]);
  });
  it('complex array', () => {
    const proxies = [
    ...
    expect(result).to.eql(proxies.map((item) => {
      const newContext = item.context.map(name => `${publicPath}${name.endsWith('/') ? name.substr(0, name.length - 1) : name}`);
